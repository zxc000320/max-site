(() => {
    const canvas = document.getElementById('Swipe_Brick_Breaker_canvas');
    ...
    ctx.lineWidth = 2;
    /* //canvas default setting */
    const C = '#800080'; // default block color
    ...
    class ball {
        constructor(x, y) {
            ...
            this.dx = FPS; // 움직이는 X 값
            this.dy = FPS; // 움직이는 Y 값
            this.isShoot = false; // 날아가고 있는지
            this.opacity = 1;
        }
        draw(c = `rgba(255, 157, 255, ${this.opacity || 1})`, x = this.x, y = this.y) {
            ctx.save();
            ctx.beginPath();
        ...
        update() {
            ...
            if (this.x + this.radius >= canvas.width || this.x <= this.radius) {
                this.x =
                    this.x + this.radius >= canvas.width ? canvas.width - this.radius : this.radius;
                this.dx = this.dx * -1;
            }
    ...
    class AddBall {
        ...
        draw() {
            ctx.save();
            ctx.strokeStyle = '#da5ada';
            ctx.fillStyle = '#da5ada';
            ctx.setLineDash([0, 0]);
            ctx.lineWidth = 3;
    ...
    // 두 점 사이의 거리를 구함
    const GetPointFromDigree = (x, y, digree, len) =>
        new Object({
    ...
    const callback = async () => {
        ...
        // }
        /* //ball */
        document.querySelector('.Swipe_Brick_Breaker_score').innerText = turn;
        document.querySelector('.Swipe_Brick_Breaker_b').innerText = Balls.length;
        Blocks = Blocks.map((v) => {
            if (v.opacity != 1) v.opacity = 1;
            return v;
        });
    ...
    const radios = () => {
        //속도 변경
        let radio = document.querySelectorAll('.Swipe_Brick_Breaker_radio');
        for (let i = 0, len = radio.length; i < len; i++) {
            radio[i].onclick = function () {
                FPS = radio[i].value * 10;
                return FPS;
            };
        }
    }; //속도 변경
    ...
    Swipe_Brick_Breaker_Game = () => {
        console.log('re');
        turn = 1;
        Move_cnt = 0; // mousemove cnt (예상경로 움직이는데 사용됨)
        Shootcnt = 0; // 날아가고있는 공의 개수
        mousestate = 0; // 0= defualt 1= mousedown 2= mousemove 3= mouseup
        should_Add = 0; // 추가해야되는 공 cnt
        (px = 0), (py = 0); // 예상경로 line
        (tx = 0), (ty = 0); // 예상경로 ball
        Balls = []; // Ball array
        Blocks = []; // 블록 array
        AddBalls = []; // 공 추가하는거 array
        fbid = null; // 바닥에 닿은 첫번째 공의 idx
        Iscallback = false;
        Isend = false;
    ...
    window.onload = () => {
        Balls.push(new ball()); //처음 공
        Blocks.push(new Block({ l: 0, t: 1, cnt: turn })); //처읍불럭1
        Blocks.push(new Block({ l: 5, t: 1, cnt: turn })); //처음블럭2
        AddBalls.push(new AddBall({ l: 1, t: 1 })); //처음공추가
        fb = Balls[0]; //처음공 첫배열 지정
        eve();
        display();
    }; //페이지 로드시 작동
})();